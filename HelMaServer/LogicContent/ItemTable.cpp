...
{
    std::string ID = _PID;
    std::string NewQuery = "SELECT no, PID, Name, Type, InvenOrder, Count, IconResData FROM helmaserver.itemtable WHERE PID = '";
    NewQuery += ID + "'";
    return NewQuery;
}
...
std::string ItemTable::InsertQueryCreate(ItemTableData& _Data) {
    ...
    std::string NewQuery = "INSERT INTO helmaserver.itemtable(PID, Name, Type, InvenOrder, Count, IconResData) VALUES('"
        + Data[1]->ToString() + "', '"
        + ConvertName.c_str() + "', '"
        + Data[3]->ToString() + "', '"
        + Data[4]->ToString() + "', '"
...
std::string ItemTable::UpdateQueryCreate(ItemTableData& _Data) 
{
    ...
    std::string NewQuery = "UPDATE helmaserver.itemtable SET PID = '"
